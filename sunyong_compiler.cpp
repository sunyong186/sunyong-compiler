...
public:
    ...
    char getOperatorChar() const { return Name[Name.size() - 1]; }
...
static std::unique_ptr<ExprAST> ParseForExpr()
{
    ...
    if (CurTok != '=')
        return LogError("expected '=' after for id");
    getNextToken();
...
static std::unique_ptr<ExprAST> ParseVarExpr()
{
    ...
    while (true)
    {
        ...
        getNextToken();
        if (CurTok != tok_identifier)
            return LogError("expected identifier list after comma");
    }
...
//===----------------------------------------------------------------------===//
// Code Generation & Context Infrastructure
//===----------------------------------------------------------------------===//
...
// ★ 구조 전면 수정: 새로운 함수 정의 시 선언부를 강제 빌드하도록 교정
Function *FunctionAST::codegen()
{
    std::string FnName = Proto->getName();
    PrototypeAST &P = *Proto;
    FunctionProtos[FnName] = std::move(Proto); // 맵에 안전하게 소유권 이전

    // 1. 기존 모듈에 존재 여부 탐색
    Function *TheFunction = TheModule->getFunction(FnName);
    if (!TheFunction)
        TheFunction = P.codegen(); // 2. 없으면 프로토타입 객체를 통해 직접 강제 생성

    if (!TheFunction)
        return nullptr;

    if (P.isBinaryOp())
        BinopPrecedence[P.getOperatorChar()] = P.getBinaryPrecedence();
...
static void InitializeModule()
{
    ...
    TheModule = std::make_unique<Module>("Sunyong Language JIT Engine", *TheContext);
...
int main()
{
    ...
    fprintf(stdout, "\n[Sunyong] 독점 컴파일 완료: %s 오브젝트 덤프 성공.\n", Filename.c_str());
    outs().flush();
